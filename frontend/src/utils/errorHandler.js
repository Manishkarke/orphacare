...
function dontionFormValidator({ weight }, setErrors) {
  ...
  if (!weight) {
    setErrors((prevErrors) => {
      return { ...prevErrors, weight: "Weight is required" };
    });
  } else if (weight < 0) {
    setErrors((prevErrors) => {
      return { ...prevErrors, weight: "Weight cannot be Negative" };
    });
  } else {
    setErrors((prevErrors) => {
...
// Error handler for donate amount
function donateMoneyValidator(donateAmount, setError) {
  if (donateAmount < 10 || donateAmount === undefined) {
    return setError("Donation Amount must be more than Rs. 10");
  } else {
    return setError("");
  }
}
...
function reportFormValidator(
  { lastSeenAddress, childEstimatedAge },
  setErrors
) {
